...
const resolvers = {
  Query: {
    ...
    obtenerEmprendimientos: async (_, { input }) => {
      const emprendimientos = await Emprendimiento.where('categoria').equals(
        input.categoria
      )
    ...
    obtenerProductos: async (_, { input }) => {
      const productos = await Producto.where('perteneceA').equals(
        input.perteneceA
      )
  ...
  Mutation: {
    ...
    autenticarUsuario: async (_, { input }) => {
      ...
      // Dar acceso a la app
      return {
        token: crearToken(existeUsuario, config.SECRET_KEY, '2hr'),
      }
    },
    ...
    actualizarCategoria: async (_, { id, input }) => {
      ...
      categoria = await Categoria.findOneAndUpdate({ _id: id }, input, {
        new: true,
      })
      return categoria
    },
    ...
    editarEmprendimiento: async (_, { id, input }) => {
      ...
      emprendimiento = await Emprendimiento.findOneAndUpdate(
        { _id: id },
        input,
        { new: true }
      )
